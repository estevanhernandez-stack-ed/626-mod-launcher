...
- [ ] **Bulk Nexus surfaces unchanged (B2b not done).** Endorse, "Refresh Nexus stats", the UPDATE chip, and bulk endorse-state sync still run through Core's `NexusClient` (App-facing plugin registry for endorse/stats from B1; Core's `NexusRefresh` for the sweep) — B2a only moved scan-time identify. Confirm these still work exactly as before.
