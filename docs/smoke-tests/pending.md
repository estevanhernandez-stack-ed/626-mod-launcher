...
- ⚠️ **KNOWN REGRESSION (B1-introduced, caught by the B2a adversarial review — B2b fixes it).** "Refresh Nexus stats" and the 24h auto-poll were moved to a per-mod `FetchMetadataAsync` loop in B1, which silently dropped two bulk features: (1) the **library-wide endorse-heart sync** (the bulk `GetUserEndorsementsAsync → ApplyEndorsements` that reflected mods endorsed on the Nexus *website*) — so a stats refresh no longer fills hearts from outside-the-launcher endorsements; and (2) the **rate-limit-aware windowed poll** — the auto-check now hits every identified mod by id (more API load) with no 429 backoff and writes the poll stamp even on a throttled/partial poll (so it won't retry for 24h). `NexusRefresh.RefreshAllAsync`/`SelectCandidates`/`ApplyEndorsements` are currently **orphaned (zero callers)**. The heart is never *wiped* (the `Endorsed: null` guard holds) — it's the *sync* and *backoff* that are gone. **B2b restores both** by adding the bulk endorse-state + updated-window reads to the `IModSource` contract (see `2026-06-18-plugin-nexus-B2-design.md`). Until then: stats-refresh enriches stats but won't sync website-endorsed hearts; the poll is per-mod.
