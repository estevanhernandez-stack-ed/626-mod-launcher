...
- ⚠️ **Known parity gap — Nexus category labels (B1-introduced, accepted).** Pre-B1, an md5/by-id identify resolved the mod's Nexus `category_id` → category name (the old `NexusClient` lazily fetched + cached a per-domain category dictionary from the game-info call). The lean plugin makes no game-info prefetch, so `NexusModSource` always returns `Category = null` and a Nexus identify no longer fills `ModMeta.Category` in FULL. Nothing in Core took it over (there is no Core category resolver). This is the one identity field that lost FULL parity in the extraction. **Pending Este's call:** accept it as a lean-plugin cost (curated-catalog categories for *known* mods still apply), or restore it as a small follow-up (plugin re-adds the per-domain category-dict fetch + maps `category_id`). Tracked here so it isn't a silent drop.
- [ ] **Live smoke — full Nexus surface still works through the plugin (FULL).** On a FULL build with the dev-signed Nexus plugin loaded and Nexus connected, the whole Nexus loop — scan-time identify, "Refresh Nexus stats" (with library-wide heart sync), the endorse heart, the UPDATE chip, manual-URL match, and the 24h auto-poll — works with parity to before the cluster deletion (**except** the category-label gap noted above). Nothing else regressed from removing Core's client.
