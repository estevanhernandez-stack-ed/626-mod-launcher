...
machine only. Our full privacy statement is provided with this submission.
...
| **10.5** privacy | No telemetry, no PII collected by us; optional OAuth sign-in with the token encrypted on-machine; full privacy statement supplied with the submission. | Point to the submitted privacy statement; note the app is fully usable signed-out and stores the Nexus token encrypted on-machine only. |
