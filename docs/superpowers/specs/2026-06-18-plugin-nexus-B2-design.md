...
- Tests of the **moved impl** (`NexusClientTests`, `NexusRequestsTests`, `CategoryTests`, `NexusPostBodyTests`, `NexusRateLimitTests`, `NexusUpdatedTests`, `NexusUserEndorsementsTests`, `NexusEndorseTests`, `NexusRefreshTests`, `NexusRefreshSweepTests`) → a **new plugin test project** `tests/ModManager.Plugin.Nexus.Tests/` referencing the plugin + Abstractions. This is also where the B1-review-requested `NexusModSource` endorse-refusal unit tests land (HttpMessageHandler-mock over `SetEndorsedAsync`).
