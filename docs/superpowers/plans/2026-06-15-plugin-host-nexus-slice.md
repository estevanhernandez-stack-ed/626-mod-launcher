...
> **Contract churn to expect (flagged by the Phase A whole-branch review — reconcile these against the live Nexus code):** the slim contract drops fields the real surface carries. Prioritize when the contract churns: (1) a **bulk user-endorsements read** on `IModSource` (heart-reality sweep — `GetUserEndorsementsAsync`; today `IModSource` has only the per-mod `SetEndorsedAsync` write, no bulk read) — and **endorse state must come only from this bulk path, never a per-mod fetch** (the DTO already enforces this: `FetchMetadataAsync` returns `Endorsed: null`); (2) a **rate-limit/budget signal** (`NexusRateLimit`/`LastRateLimit`) so the sweep can back off before a 429; (3) **identity/credit fields** md5-identify produces (Title/Author/Image/Category/ModUrl/Donate/`ContainsAdultContent`/`NexusFileId`) — widen `SourceModMetadata` or add a separate identify-metadata DTO so the row builder still gets them without a second round-trip; (4) the update-window timestamps (`NexusUpdateEntry.LatestFileUpdate`) the debounced auto-check uses to narrow the batch (`IsUpdateAvailableAsync` currently collapses to a per-mod bool).
