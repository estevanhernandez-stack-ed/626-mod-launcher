...
public class NexusRefreshSweepTests
{
    private sealed class FakeSource : IModSource
    {
        ...
        private readonly Func<Task<IReadOnlyList<SourceEndorsement>>>? _getEndorsements;
        public int FetchCalls { get; private set; }
        public int GetEndorsementsCalls { get; private set; }

        public FakeSource(
            Func<SourceModRef, Task<SourceModMetadata?>> fetch,
            Func<Task<IReadOnlyList<SourceEndorsement>>>? getEndorsements = null)
        {
            _fetch = fetch;
        ...
        public Task<IReadOnlyList<SourceEndorsement>> GetUserEndorsementsAsync()
        {
            ...
            return _getEndorsements is null
                ? Task.FromResult<IReadOnlyList<SourceEndorsement>>(Array.Empty<SourceEndorsement>())
                : _getEndorsements();
        }
    ...
    public void SelectCandidates_keeps_only_ids_in_set_with_newer_file_update()
    {
        ...
        var entries = new[]
        {
            new SourceUpdateEntry(1, After(2)),
            new SourceUpdateEntry(2, Before(2)),
            new SourceUpdateEntry(99, After(5)),                   // unrelated id
        };
    ...
    public void SelectCandidates_uses_each_mods_InstalledUtc_over_the_passed_baseline()
    {
        ...
        var entries = new[] { new SourceUpdateEntry(1, update) };
    ...
    public void SelectCandidates_resolves_id_from_url()
    {
        ...
        var entries = new[] { new SourceUpdateEntry(555, after) };
    ...
    public async Task RefreshAllAsync_refreshes_each_resolvable_mod_and_counts_updates()
    {
        ...
        var fake = new FakeSource(r => Task.FromResult<SourceModMetadata?>(r.ModId switch
        {
            1 => new SourceModMetadata(50, null, "2.0", null, null),
            2 => new SourceModMetadata(9, null, "3.0", null, null),
            _ => null,
        }));
    ...
    public async Task RefreshAllAsync_stops_on_rate_limit_and_returns_partial_progress()
    {
        ...
        var fake = new FakeSource(r =>
        {
            if (r.ModId == 2) throw new SourceRateLimitException();
            return Task.FromResult<SourceModMetadata?>(new SourceModMetadata(null, null, "2.0", null, null));
        });
        ...
        Assert.Equal(2, fake.FetchCalls);           // stopped at the 429, never reached #3
    }
    ...
    // ---------- RefreshAllAsync + endorsement sweep ----------
    ...
    public async Task RefreshAllAsync_fetches_endorsements_once_and_applies_to_returned_metas()
    {
        ...
        var fake = new FakeSource(
            r => Task.FromResult<SourceModMetadata?>(r.ModId switch
            {
                1 => new SourceModMetadata(50, null, "2.0", null, null),
                2 => new SourceModMetadata(9, null, "3.0", null, null),
                _ => null,
            }),
            () => Task.FromResult<IReadOnlyList<SourceEndorsement>>(new[]
            {
                new SourceEndorsement(1, "eldenring", "Endorsed"),
                new SourceEndorsement(2, "eldenring", "Abstained"),
            }));
    ...
    public async Task RefreshAllAsync_endorsement_rate_limit_does_not_abort_the_stats_sweep()
    {
        ...
        var fake = new FakeSource(
            _ => Task.FromResult<SourceModMetadata?>(new SourceModMetadata(null, null, "2.0", null, null)),
