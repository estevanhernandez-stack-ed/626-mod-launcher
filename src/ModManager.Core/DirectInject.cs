...
public static class DirectInject
{
    ...
    /// <summary>
    /// Cascade the WHOLE DLL-mod-loader stack off or on in one reversible, atomic action: the loader
    /// (dinput8.dll) PLUS every mod it runs from its <c>mods\</c> folder. Composes the per-mod
    /// <see cref="Disable"/>/<see cref="Enable"/> primitives (which each give per-unit atomicity) under
    /// an OUTER rollback that gives the CROSS-unit atomicity they individually lack: if any unit fails
    /// mid-cascade, every unit already moved this batch is reversed in the opposite direction and
    /// re-throws. The rollback is BEST-EFFORT — a file still OS-locked (the game running) can defeat
    /// the reverse move just as it defeated the forward one, so the play folder ends "as fully-on (off
    /// path) / fully-off (on path) as the locks allowed." Nothing is ever deleted, so any residual
    /// partial state is non-destructive and re-converges on the next toggle once the game is closed.
    /// No <c>File.Delete</c>; every move routes through the same reversible <c>MoveAny</c>/<c>SafeMove</c>
    /// path as the primitives.
    /// <para>OFF disables the loader FIRST: the dominant real failure is the game running with
    /// dinput8.dll OS-locked, which then trips on unit 0 before any hosted mod moves — rollback is a
    /// no-op with the play folder already fully-on. ON restores the loader LAST so the proxy never
    /// goes live before its hosted mods are back (the silent-non-loading window).</para>
    /// </summary>
    public static void SetLoaderEnabled(string playFolder, string holdingRoot, bool enabled)
    {
        if (enabled) CascadeOn(playFolder, holdingRoot);
        else CascadeOff(playFolder, holdingRoot);
    }

    private static void CascadeOff(string playFolder, string holdingRoot)
    {
        var loader = Detect(FolderFiles(playFolder), FolderDirs(playFolder)).FirstOrDefault(m => m.Name == LoaderName);
        if (loader is null) return; // no loader installed — nothing to cascade

        var modsDir = Path.Combine(playFolder, ModsDir);
        var hosted = Directory.Exists(modsDir)
            ? DetectLoaderMods(FolderFiles(modsDir), FolderDirs(modsDir))
            : Array.Empty<DirectInjectMod>();

        // Loader FIRST (cheapest rollback when the game holds dinput8.dll), then its hosted mods.
        var batch = new List<DirectInjectMod> { loader };
        batch.AddRange(hosted);

        // Refuse up-front (move NOTHING) on a slug collision — two units sharing one holding dir would
        // clobber each other's meta and make the ON restore lose files. Untouched is most-recoverable.
        var bySlug = batch.GroupBy(u => EnginePresets.Slugify(u.Name)).FirstOrDefault(g => g.Count() > 1);
        if (bySlug is not null)
            throw new InvalidOperationException(
                $"Can't disable the {LoaderName}: \"{string.Join("\" and \"", bySlug.Select(u => u.Name))}\" map to the same holding folder. Rename one and retry.");

        // Refuse up-front on a stale holding dir (a prior toggle that didn't finish) — Disable would
        // silently merge into it and overwrite its meta.
        foreach (var u in batch)
        {
            var slugDir = Path.Combine(holdingRoot, EnginePresets.Slugify(u.Name));
            if (Directory.Exists(slugDir))
                throw new InvalidOperationException(
                    $"Can't disable the {LoaderName}: a previous toggle didn't finish — resolve \"{slugDir}\" first.");
        }

        var disabled = new List<DirectInjectMod>();
        try
        {
            foreach (var u in batch)
            {
                Disable(playFolder, holdingRoot, u);
                disabled.Add(u);
            }
        }
        catch (Exception e)
        {
            // OUTER rollback: re-enable everything already disabled this batch, in reverse, best-effort
            // (a still-locked file must not abort restoring the rest). Then re-throw — never swallow.
            for (var i = disabled.Count - 1; i >= 0; i--)
                try { Enable(playFolder, holdingRoot, disabled[i].Name); } catch { /* best effort */ }
            throw new InvalidOperationException($"Couldn't disable the {LoaderName} ({e.Message}) — is the game running?", e);
        }
    }

    private static void CascadeOn(string playFolder, string holdingRoot)
    {
        // Source of truth is HOLDING (after OFF the play mods\ folder is empty). Snapshot now — Enable
        // clears holding dirs as it goes, and the rollback needs the unit records to re-disable.
        var disabledUnits = ListDisabled(holdingRoot);
        var loaderUnit = disabledUnits.FirstOrDefault(m => m.Name == LoaderName);
        var hostedUnits = disabledUnits.Where(m => m.Name != LoaderName).ToList();
        if (loaderUnit is null && hostedUnits.Count == 0) return; // nothing disabled — no-op

        // Hosted FIRST, loader LAST (so the proxy only goes live once its mods are restored).
        var order = new List<DirectInjectMod>(hostedUnits);
        if (loaderUnit is not null) order.Add(loaderUnit);

        var byName = disabledUnits.ToDictionary(u => u.Name);
        var enabledNames = new List<string>();
        try
        {
            foreach (var u in order)
            {
                Enable(playFolder, holdingRoot, u.Name);
                enabledNames.Add(u.Name);
            }
        }
        catch (Exception e)
        {
            // OUTER rollback: re-disable everything already restored this batch, in reverse, best-effort.
            for (var i = enabledNames.Count - 1; i >= 0; i--)
                if (byName.TryGetValue(enabledNames[i], out var unit))
                    try { Disable(playFolder, holdingRoot, unit); } catch { /* best effort */ }
            throw new InvalidOperationException($"Couldn't enable the {LoaderName} ({e.Message}) — is the game running?", e);
        }
    }

    // Top-level file / dir basenames of a folder (the shape Detect/DetectLoaderMods want), tolerant of
    // an unreadable folder. Mirrors DirectInjectListing.Names.
    private static IReadOnlyList<string> FolderFiles(string folder) => BaseNames(folder, Directory.GetFiles);
    private static IReadOnlyList<string> FolderDirs(string folder) => BaseNames(folder, Directory.GetDirectories);
    private static IReadOnlyList<string> BaseNames(string folder, Func<string, string[]> list)
    {
